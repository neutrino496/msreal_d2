...
int provera(struct storage s[], int position){
	int true=-1, i;
	for(i=0;i<10;i++)
	{	if(i != position){
...
{	
	...
	sort(s);
	while(position < 10 && endRead == 0) 
	{
		//if ((strcmp(s[position].name, " "))  && strcmp(s[position].surname, " ") && (s[position].index!=0))
		//{
			len = scnprintf(buff,BUFF_SIZE , "%s %s %d %d \n", s[position].name, s[position].surname, s[position].index, s[position].points);
			ret = copy_to_user(buffer, buff, len);
			printk(KERN_INFO "%s %s %d %d", s[position].name, s[position].surname, s[position].index, s[position].points);
			if(ret)
				return -EFAULT;
		//}
		position++;
...
// semafor
	...
	ret = sscanf(buff,"%s %s %d %d",ime, prezime, &br_ind, &bodovi);
	if(ret==4)// parameters parsed in sscanf
	{
		if(position < 10){
			strcpy(s[position].name, ime);
			strcpy(s[position].surname, prezime);
			s[position].index = br_ind;
			s[position].points = bodovi;
			if(position>0){
				if(provera(s, position)) {
					s[provera(s,position)-1].points=bodovi;
                               		printk(KERN_INFO "Succesfully changed value in position %d \n", provera(s, position));
					strcpy(s[position].name, " ");
                        		strcpy(s[position].surname, " ");
                        		s[position].index = 0;
                        		s[position].points = 0;
				} else {
...
void izbrisi(struct file *pfile, const char __user *buffer, size_t length, loff_t *offset)
{
	...
	if(ret)
		return -EFAULT;
	buff[length-1] = '\0';
	ret = sscanf(buff,"izbrisi = %s %s %d",ime, prezime, &br_ind);
	if(ret==3)// parameters parsed in sscanf
	{
		for(i=0;i<10;i++){
			if (!(strcmp(s[i].name, ime)) && !(strcmp(s[i].surname,prezime)) && (s[i].index == br_ind)){
				strcpy(s[i].name, " ");
				strcpy(s[i].surname, " ");
				s[i].index=0;
				s[i].points=0;

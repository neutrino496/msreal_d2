...
{	
	...
	{
		if ((strcmp(s[position].name, " "))  && strcmp(s[position].surname, " ") && (s[position].index==0) && (s[position].points==0))
		{
...
void izbrisi(struct storage s[], struct file *pfile, const char __user *buffer, size_t length, loff_t *offset)
{
	int ret;
	int br_ind, bodovi, i;
	char ime[11];
